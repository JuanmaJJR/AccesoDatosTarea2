...
$query = "SELECT jugadores.ID,jugadores.Nombre,jugadores.Apellido,jugadores.Posicion,jugadores.ID_Equipo,equipos.ID as IDEquipo, equipos.nombre as NombreEquipo 
FROM jugadores 
...
if (isset ( $result ) && $result) { // Si pasa por este if, la query está está bien y se obtiene resultado
	...
	if ($result->num_rows > 0) { // Aunque la query esté bien puede no obtenerse resultado (tabla vacía). Comprobamos antes de recorrer
		...
		while ( $row = $result->fetch_assoc () ) {
			...
			$arrJugador["equipo"] = $row["ID_Equipo"];
			$arrJugador["apellido"] = $row["Apellido"];
            $arrJugador["posicion"] = $row["Posicion"];
            $arrJugador["idEquipo"] = $row["IDEquipo"];
            $arrJugador["nombreEquipo"] = $row["NombreEquipo"];
